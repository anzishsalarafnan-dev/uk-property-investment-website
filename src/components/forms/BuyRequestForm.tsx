...
const schema = z.object({
  ...
  budgetMax: z.number().positive("Enter a valid budget"),
...
export default function BuyRequestForm({ cities }: { cities: { slug: string; name: string }[] }) {
  ...
  async function onSubmit(data: FormData) {
    setStatus("loading");
    try {
      const recaptchaToken = await getToken("buy_submit");
      const res = await fetch("/buy/submit", {
        method: "POST",
  ...
  if (status === "success") {
    return (
      <p className="rounded-md bg-emerald-50 p-4 text-sm font-medium text-emerald-700">
        Thanks! Your request will appear on the site once reviewed by our team.
      </p>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-lg space-y-4">
      ...
      <div>
        ...
        <input {...register("budgetMax", { valueAsNumber: true })} type="number" className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
        {errors.budgetMax && <p className="mt-1 text-xs text-red-600">{errors.budgetMax.message}</p>}
      </div>
      <div>
        <label className="text-sm font-medium text-slate-700">Bedrooms wanted (optional)</label>
        <input {...register("bedroomsWanted", { valueAsNumber: true })} type="number" className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
      </div>
      <div>
        <label className="text-sm font-medium text-slate-700">Other requirements (optional)</label>
        <textarea {...register("requirements")} rows={4} className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
      </div>
